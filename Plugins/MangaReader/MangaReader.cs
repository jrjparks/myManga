...
namespace MangaReader
{
    ...
    [PluginVersion("0.0.3")]
    public class MangaReader : IMangaPluginBase, IMangaPlugin
    {
        #region IMangaPlugin Vars
        public string SiteName { get { return "MangaReader"; } }
        public string SiteURLFormat { get { return @"mangareader\.net"; } }
        public SupportedMethods SupportedMethods { get { return SupportedMethods.All; } }
        #endregion
        
        [EditorBrowsable(EditorBrowsableState.Never), Browsable(false)]
        public class MangaReaderChapterJSON
        {
            public class Item
            {
        ...
        public MangaArchiveInfo LoadChapterInformation(String ChapterPath)
        {
            ...
            OnProgressChanged(3);
            Match vcscMatch = Regex.Match(ChapterPath, @"(http://www\.mangareader\.net/)([a-z0-9\-]+)/(?<Chapter>\d+)"), PageMatch;
            String PageHTML, Chapter = vcscMatch.Groups["Chapter"].Value,
                RegExImageSearch = @"src=.(?<OnlinePath>(http://i\d{1,4}\.mangareader\.net/)([a-z0-9\-]+)/(\d+)/)(?<File>[a-z0-9\-]+\.jpg)",
        ...
        public MangaInfo LoadMangaInformation(String InfoPage)
        {
            ...
            using (WebClient WebClient = ConfigureWebClient(SiteRefererHeader))
            {
                ...
                try { MI.ID = UInt32.Parse(Regex.Match(InfoPage, @"mangareader\.net/(?<ID>\d+)/[\w-]*.html").Groups["ID"].Value); }
        ...
        public String ManipulateMangaData(MangaData MangaData, DataType ManipulationType)
        {
            switch (ManipulationType)
            {
                case DataType.ChapterInformation:
                    return String.Format("http://www.mangareader.net/{0}/{1}", NameToUrlName(MangaData.Name), MangaData.Chapter);

                case DataType.MangaInformation:
                    return String.Format("http://www.mangareader.net/{0}", NameToUrlName(MangaData.Name));
            }
            return String.Empty;
        }
        ...
        public SearchInfoCollection Search(String Text, Int32 Limit)
        {
            ...
            String SearchPath = String.Format("http://www.mangareader.net/actions/search/?q={0}&limit={1}", Text, Limit),
                _Data;
            String[] _DataArray;
            OnProgressChanged((Int32)(Progress += 5));

            using (WebClient GWC = ConfigureWebClient(SiteRefererHeader))
            {
                GWC.Encoding = Encoding.UTF8;
                using (Stream _s = new MemoryStream(GWC.DownloadData(SearchPath)))
                {
                        ...
                        while ((_Data = _sr.ReadLine()) != null && SearchCollection.Count < Limit)
                        {
                            ...
                            SearchCollection.Add(
                                new SearchInfo(
                                    ...
                                    _DataArray[1],
                                    _DataArray[3],
                                    ConvertOldInformationUrlToNew(String.Format("http://www.mangareader.net{0}", _DataArray[4])),
        ...
        private String ConvertOldInformationUrlToNew(String OldUrl)
        {
            ...
            if (OldMatch.Groups["Title"].Success)
                return String.Format("http://www.mangareader.net/{0}", OldMatch.Groups["Title"].Value);
            return OldUrl;
        }
        private String ConvertOldChapterUrlToNew(String OldUrl)
        {
            ...
            if (OldMatch.Groups["Title"].Success && OldMatch.Groups["Chapter"].Success)
                return String.Format("http://www.mangareader.net/{0}/{1}", OldMatch.Groups["Title"].Value, OldMatch.Groups["Chapter"].Value);
            return OldUrl;
        }
        ...
        private ChapterEntryCollection ChapterList(UInt32 ID)
        {
            ...
            using (WebClient WebClient = ConfigureWebClient(SiteRefererHeader))
            {
                ...
                MangaReaderChapterJSON _json = JSON.Instance.ToObject<MangaReaderChapterJSON>(_ChapterContent);
                MangaReaderChapterJSON.Item _Item;
                ...
                for (Int32 i = 0; i < _json.data.Length; ++i)
                {
                    ...
                    _Chapters.Add(
                        new ChapterEntry()
                        {
                            ...
                            UrlLink = ConvertOldChapterUrlToNew(String.Format("http://www.mangareader.net{0}", _Item.chapterlink.Replace(@"\/", "/")))
                        });
